...
async function downloadMedia(url, savePath) {
  let indexDestroy = [];url = ('' + url).split('/');
  ...
  if(getMetaData === "Error")return getMetaData;
  ...
  if (getType == "image") {
    ...
    result.file = `./results/insta/${getMetaData.shortcode_media.shortcode}.jpg`
    result.type = 'Image'
  } else {
    await download(getMetaData.shortcode_media.video_url,`${getMetaData.shortcode_media.shortcode}.mp4`,savePath);
    result.file =  `./results/insta/${getMetaData.shortcode_media.shortcode}.mp4`
    result.type = 'Video'
  }
